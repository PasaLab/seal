...
import edu.stanford.nlp.trees.{TypedDependency, GrammaticalStructure, Tree, PennTreebankLanguagePack}
import scala.collection.JavaConversions._
import edu.nju.pasalab.mt.extraction.util.parser.NLP._
...
case class Sentence(word:Array[String]) extends CoreLabelSeq {
  ...
  //case class StringSeqHolder(seq : Seq[String])
  
  override def equals(a:Any):Boolean = {
    def seqMatch(s : Seq[String]) : Boolean = {
      s.length == word.length && s.zip(word).forall { case (a, b) => a.equals(b) }
    }
    a match {
      case (s : Sentence) =>
        for (id1 <- this.id;
        ...
        seqMatch(s.word)
      case (s : Seq[String @unchecked])  => seqMatch(s)
      case _ => false
    }
  }
