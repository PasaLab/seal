...
object reorderMerge {
  def main(args: Array[String]) {
    ...
    val phrase_table = spark.sparkContext.textFile(ep.rootDir + "/phrase.C2E").map(x => {
      val sp = x.split(" \\|\\|\\| ")
    ...
    val reorder_table = spark.sparkContext.textFile(ep.rootDir + "/phrase.reordering.C2E").map(x => {
      val sp = x.split(" \\|\\|\\| ")
      val key = new StringBuilder(128)
      key.append(sp(0)).append(" ||| ").append(sp(1))
      (key.toString(), sp(2))
    ...
    val join = phrase_table.join(reorder_table).mapPartitions(part => {
      part.map(elem => {
        val key = new StringBuilder(128)
...
/*    CommonFileOperations.deleteIfExists(ep.rootDir + "/merged.phrase.C2E")
    ...
    for (elem <- join.collect()) {
      val value1 = new StringBuilder(300)
      val value2 = new StringBuilder(300)
      ...
      value1.append(value(2)).append(" ").append(value(3)).append(" ")
        .append(value(1)).append(" ").append(value(2)).append(" ").append(value(4))
