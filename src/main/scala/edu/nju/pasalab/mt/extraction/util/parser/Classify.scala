...
import edu.stanford.nlp.classify.{LinearClassifierFactory, LogPrior, RVFDataset}
...
class Mapping[I : ClassTag, O : ClassTag](map:Map[I,(O,Float)]) {
  import Mapping.{toCounter,defaultFeatures}

  def scorer[F :ClassTag](featurizer:I=>Iterable[F]):I=>Map[O,Double] = {
    // -- Create Dataset
  ...
  def classifier[F : ClassTag](featurizer:I=>Iterable[F]):Classifier[I,O]
...
object Mapping {
  def toCounter[X :ClassTag, F : ClassTag](map:Iterable[X]):Counter[F] = {
  ...
  def apply[I : ClassTag, O : ClassTag, X : ClassTag](map:Map[I,X]):Mapping[I,O] = {
...
class Ensemble[I : ClassTag](members:Seq[I=>Boolean], dat:Option[Map[I,(Boolean,Float)]]) {
  // -- Get Data
  if (!dat.isDefined) {
